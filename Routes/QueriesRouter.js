...
// Create a queries
QueriesRouter.post('/', middleware.isAdmin, async (req, res, next) => {
  ...
  queriesData.save()
    .then((queries) => {
      res.status(200).json({ success: true, message: "Query is created successfully" });
    }, (err) => next(err))
    .catch((err) => {
      next(err);
    });
...
// Get all queries
QueriesRouter.get('/', middleware.isAdmin, async (req, res, next) => {
  Queries.find().populate("developer")
    .then((queries) => {
...
QueriesRouter.get('/:id', middleware.isAdmin, async (req, res, next) => {
  const { id } = req.params;

  Queries.findById(id).populate("developer")
    .then((queries) => {
      if (!queries) {
...
QueriesRouter.patch('/:id', middleware.isAdmin, async (req, res, next) => {
  try {
...
QueriesRouter.delete('/:id', middleware.isAdmin, async (req, res, next) => {
  const { id } = req.params;
...
// send email
QueriesRouter.post("/sendEmail", middleware.isAdmin, async (req, res, next) => {
